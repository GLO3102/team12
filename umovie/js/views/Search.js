 ...
 */
/**
 * Created by Utilisateur on 2015-12-02.
 */
define([
	'jquery',
	'underscore',
	...
	'collections/TvshowGenreCollection'], function ($, _, Backbone, select2, Template, SearchCollection, MoviesGenres, TvshowGenres) {
	...
	var SearchView = Backbone.View.extend({
		...
		render: function () {
			...
			var html = template({filter: self.filter, results:self.results, moviesGenres:self.moviesGenres, tvshowGenres:self.tvshowGenres, users: self.users});
			this.$el.html(html);
		...
		selectMovieGenre: function (evt) {
			...
			if(mg != 0 && this.filter.q){
				...
				//window.app.router.navigate(url,{trigger:true,replace:true});
